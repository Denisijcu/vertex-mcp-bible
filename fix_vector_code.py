...
# 2. Regenerar main.py con ChromaDB
main_code = """from fastapi import FastAPI, Depends, HTTPException
import uvicorn
import chromadb
...
# Inicializar ChromaDB (Base Vectorial en memoria)
    ...
    try:
        if request.method == "add_document":
            ...
            return MCPBaseResponse(
                id=request.id, 
                result={"status": "Contexto vectorizado exitosamente", "doc_id": doc_id}
            )
...
print("[+] Nodo Vector y ChromaDB regenerados en UTF-8 puro.")